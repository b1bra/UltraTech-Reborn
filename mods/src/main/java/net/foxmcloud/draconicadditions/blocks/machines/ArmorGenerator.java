...
public class ArmorGenerator extends BlockBCore implements ITileEntityProvider {
	...
	public void randomDisplayTick(World worldIn, int x, int y, int z, Random rand) {
		...
		if (tile instanceof TileArmorGenerator) {
			active = ((TileArmorGenerator) tile).active;
		}
		if (active) {
			...
			if (rand.nextDouble() < 0.1D) {
				worldIn.playSoundEffect(x + 0.5D, y, z + 0.5D, "random.fizz", 1.0F, 1.0F);
			}

			switch (enumfacing) {
			case WEST:
				worldIn.spawnParticle("crit", d0 - d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D);
				worldIn.spawnParticle("magicCrit", d0 - d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D);
				break;
			case EAST:
				worldIn.spawnParticle("crit", d0 + d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D);
				worldIn.spawnParticle("magicCrit", d0 + d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D);
				break;
			case NORTH:
				worldIn.spawnParticle("crit", d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D);
				worldIn.spawnParticle("magicCrit", d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D);
				break;
			case SOUTH:
				worldIn.spawnParticle("crit", d0 + d4, d1, d2 + d3, 0.0D, 0.0D, 0.0D);
				worldIn.spawnParticle("magicCrit", d0 + d4, d1, d2 + d3, 0.0D, 0.0D, 0.0D);
