...
public class ItemDrainer extends BlockBCore implements ITileEntityProvider {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, DraconicAdditions.instance, GUIHandler.GUIID_ITEM_DRAINER, world, x, y, z);
		}
		return true;
	}
	...
	public void randomDisplayTick(World worldIn, int x, int y, int z, Random rand) {
		...
		if (tile instanceof TileItemDrainer) {
			active = ((TileItemDrainer) tile).active;
		}
		if (active) {
			...
			switch (enumfacing) {
			case WEST:
				worldIn.spawnParticle("happyVillager", d0 - d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D);
			...
			case EAST:
				...
				break;
			case NORTH:
				worldIn.spawnParticle("happyVillager", d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D);
				worldIn.spawnParticle("reddust", d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D);
				break;
			case SOUTH:
				worldIn.spawnParticle("happyVillager", d0 + d4, d1, d2 + d3, 0.0D, 0.0D, 0.0D);
				worldIn.spawnParticle("reddust", d0 + d4, d1, d2 + d3, 0.0D, 0.0D, 0.0D);
